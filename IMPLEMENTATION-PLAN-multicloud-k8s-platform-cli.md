...
**Acceptance criteria:** a cluster's `addons.yaml` correctly resolves to the full `tier-small` set with a test override patch applied without duplicating the base. **Met at the code level**: `internal/catalog/tiers_test.go` asserts every named addon is present, override merging is verified (`internal/catalog` and `internal/orchestrator` tests), and `platform-profiles`-repo resolution is verified against a fake (`internal/catalog/repo_resolver_test.go`). A real `platform-profiles` repo is still open — this milestone's one remaining item is standing up the actual org/repo, a real infra action this pass didn't attempt.
