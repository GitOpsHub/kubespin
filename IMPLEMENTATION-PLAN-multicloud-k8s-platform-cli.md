...
- [x] Stand up `platform-profiles` repo (AWS-hosted org, same GitHub Enterprise)
  - Live at `GitOpsHub/platform-profiles` (private), seeded with `profiles/tier-small/1.0.0.yaml`, `profiles/tier-standard/1.0.0.yaml`, and `profiles/tier-regulated/1.0.0.yaml`, generated directly from the builtin catalog definitions in `internal/catalog/catalog.go` and `tiers.go` so they resolve identically through `catalog.RepoResolver`. `--profiles-repo platform-profiles` is now backed by a real repo instead of a fake.
- [x] Define `tier-small` profile: CNI (cloud default / Cilium), cert-manager, Gateway API impl (per-cloud), ESO, Kyverno (baseline), Cluster Autoscaler, kube-prometheus-stack, Fluent Bit, OpenCost, ExternalDNS, fleet-status-reporter
  - `internal/catalog`'s builtin `tier-small` now carries every named addon, built from real public Helm charts (`internal/catalog/catalog.go`), so it renders and installs as-is rather than standing in for a shape still to be decided. `core.AddonRef` now carries a `Providers` constraint (used by tier-standard's `karpenter`), so per-provider gating is no longer a missing mechanism — but Gateway API's per-cloud controller choice and Cilium-vs-managed-CNI selection are still one addon entry each with no per-provider variant defined; a per-cluster override (`core.AddonOverride`) can disable either today. Moving this into an actual `platform-profiles` repo (this milestone's other still-open item) is a real infra action, not a code change.
...
**Acceptance criteria:** a cluster's `addons.yaml` correctly resolves to the full `tier-small` set with a test override patch applied without duplicating the base. **Met**: `internal/catalog/tiers_test.go` asserts every named addon is present, override merging is verified (`internal/catalog` and `internal/orchestrator` tests), `platform-profiles`-repo resolution is verified against a fake (`internal/catalog/repo_resolver_test.go`), and the real `GitOpsHub/platform-profiles` repo is now seeded and resolvable via `--profiles-repo`.
...
- [x] Argo CD Helm-as-library install (`helm.sh/helm/v3/pkg/action`), self-referential — points at the cluster's own repo `/addons.yaml`-resolved manifests
  - Implemented: `internal/argocd.HelmInstaller` (`install.go`) runs `helm upgrade --install` semantics via `pkg/action` against a `*rest.Config` — install if `action.NewHistory` finds no release, upgrade otherwise. The `*rest.Config` acquisition this was blocked on is now real per cloud: `provisioner.RESTConfigProvisioner`, implemented by all three `ClusterProvisioner`s (`internal/provisioner/{aws,gcp,azure}/kubeauth.go`) — AWS mints a presigned STS `GetCallerIdentity` token the way `aws eks get-token` does, GCP uses an Application Default Credentials OAuth token, Azure parses the kubeconfig `ListClusterUserCredentials` returns (the same one `az aks get-credentials` writes). The root Application is applied directly via a new `argocd.KubeApplier` (`apply.go`, client-go dynamic client + server-side apply), never committed to the repo it manages. Accepted the ~450-line `go.sum` cost this was deferred over: `helm.sh/helm/v3` plus `k8s.io/client-go`/`k8s.io/apimachinery` are now direct dependencies. Live-cluster install/upgrade itself has no test coverage — `action.Install.Run`/`action.Upgrade.Run` both require a reachable API server before doing anything, the same live-infra gap as every cloud SDK call in this codebase — but the release-exists branch, the REST-client-getter adapter, and the chart-locate wiring are unit tested.
- [x] App-of-apps pattern: one root Application discovers per-addon Applications (each addon = independent Application, independent sync/failure)
  - `internal/argocd`: `RenderRootApplication` (points at `apps/` in the cluster's own repo, never committed there itself) + `RenderAddonApplications` (one manifest per addon under `apps/`, each with its own automated prune/self-heal sync policy). Now fully wired: `internal/orchestrator`'s new `installArgoCDStep` (PhaseRepoPushed) applies the root Application via `argocd.KubeApplier` and commits the per-addon Applications via the new `repo.ReconcileAppOfApps`, which hashes the rendered set against a new `AppsHash` field in `.state.yaml` so a no-change apply commits nothing here either — matching the addons.yaml no-op guarantee M3 established. `repo.Provisioner.Clone` had to grow a real fix to make that idempotent: it now lists `apps/`'s actual directory contents (a GitHub Contents API directory listing) rather than only ever checking a fixed set of paths, so previously-committed Application files are tracked in `Checkout` instead of looking "new" on every push.
...
**Acceptance criteria:** running `mycli apply` on a fresh cluster results in all `tier-small` addons `Synced`/`Healthy` in Argo CD with no manual steps, on all three clouds, in both public and private access modes. **Not yet verifiable against a real cluster** (same live-infra caveat as M2's own acceptance criteria) — but every piece of code the acceptance criteria depends on is now implemented and wired end-to-end: `apply` installs Argo CD, applies the root Application, and commits app-of-apps for every cluster, not just up to a no-op placeholder.
...
- [x] `tier-standard` profile adds: Velero, Falco, Argo CD (already present as bootstrap, now tracked as catalog entry too), Karpenter (EKS)
  - `internal/catalog/tiers.go`. Built as tier-small's addon set plus these four, via `withAddons` rather than a second addon list — a real superset, not a parallel definition that could drift from tier-small. Karpenter is now provider-gated: `core.AddonRef.Providers` restricts it to `core.ProviderAWS`, and `core.Profile.ForProvider` (wired into `internal/orchestrator.resolveProfile` and `internal/fleet.UpdateOne`) drops it before a GCP/Azure cluster's addons.yaml or an override patch ever sees it. Gateway API's per-cloud controller choice (tier-small) is a separate, still-open decision — the mechanism now exists, but no addon yet models "pick GKE's Gateway controller vs. Cilium's."
...
- [x] `fleet update --profile X --component Y --version Z`: rate-limited worker pool, patches every matching cluster's repo, staged in waves (canary tier first)
  - `internal/fleet.Update` + `kubespin fleet update`. Reuses `catalog.Merge` and `repo.ReconcileAddons` rather than reimplementing override/commit logic — an update wave is just many clusters each getting one addon override applied on top of whatever they already carry (`updateRecord` reads each cluster's own `cluster.yaml` first specifically so an existing override patch survives the wave). One cluster's failure doesn't abort its own wave (`UpdateResult` per cluster). **Canary-first staging is implemented**: `--canary-count N` updates the first N clusters (ordered deterministically by cluster ID) as their own wave first; a canary failure marks the rest of the fleet `Skipped` rather than continuing past it, and only a clean canary wave rolls to the remaining clusters. `--canary-count 0` (the default) preserves the old single-wave behavior. `--profile` is still the only way to further scope a wave by hand.
- [x] `fleet audit`: scheduled job, cloud-SDK describe calls per cluster, diffs live infra vs `cluster.yaml`, writes findings to Fleet Registry
  - `internal/fleet.Audit` + `kubespin fleet audit`. Read-only against infra and repos (never calls `Reconcile` or `Push`) — audit surfaces drift for a human or a deliberate `apply` to act on, not to silently correct. **Findings are now written to the Fleet Registry**: `registry.Record` gained `Findings []string` / `FindingsAt time.Time`, written via a new `Registry.RecordFindings` (no version check, the same reasoning `Touch`/`RecordOIDCIssuer` use — audit metadata isn't a phase transition). The schema decision made: one row per cluster holding its *latest* audit result (not one row per finding or a history), consistent with how `LastReportedAt` already summarizes "most recent," not "every."  `fleet status` and the new `fleet dashboard` both surface it.
- [x] Fleet dashboard (reads Fleet Registry): sync status, drift findings, staleness, correlated by cluster ID and commit SHA
  - `internal/fleet.Dashboard` + `kubespin fleet dashboard`, which renders a single self-contained static HTML file (inline CSS, no external assets, no server) from the same registry data `fleet status` reads. Rows correlate by cluster ID — the key the registry and every cluster's own repository already share. **Commit SHA correlation is deliberately not included**: the registry does not track a commit SHA (that lives in each cluster's own `.state.yaml`), and adding it here would mean a repository read per cluster this command otherwise has no need to make.
