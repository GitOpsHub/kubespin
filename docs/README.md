...
**Milestone M0 (foundations) is complete.** What exists and works:
...
**Everything else is a stub.** `apply`, `delete`, `fleet update`, `fleet audit`,
and `fleet status` parse their flags and then exit 3 with "not implemented yet".
They fail loudly on purpose: a stub that exited 0 would imply it had done
something.

M1 is next — the Fleet Registry client, the lease primitive, and the
orchestrator that sequences `apply`. See
