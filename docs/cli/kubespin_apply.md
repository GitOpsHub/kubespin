...
```text
  ...
  -h, --help                        help for apply
      ...
      --size string                 cluster size: small, medium, or large — determines the default addon set. Argo CD ships at every size, plus cluster-autoscaler on aws (GKE and AKS autoscale their node pools natively); medium adds Velero+Falco, large raises Pod Security policies to restricted (audit mode) and adds an OTel collector (default "small")
