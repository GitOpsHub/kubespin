...
- **Cloud-native workload identity where a cluster addon needs cloud permissions** — AWS's EBS/EFS CSI drivers get IRSA roles provisioned during the cluster reconcile (`internal/provisioner/aws/cluster.go`), scoped by both `sub` and `aud`. No static credentials in clusters, and no cross-cloud identity interface: each provisioner binds what its own cluster creation installs.
