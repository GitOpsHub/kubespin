...
// NetworkProvisioner resolves, creates, and deletes the VPC network a cluster lives in.
...
// deleteEgressFirewall removes the egress rule older kubespin versions
// created. Nothing creates one now, but a legacy cluster's network cannot be
// deleted while the rule still references it.
